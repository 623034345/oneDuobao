...
- (instancetype)initWithFrame:(CGRect)frame data:(NSArray *)data addTime:(NSString *)time{
...
- (instancetype)initWithFrame:(CGRect)frame addTime:(NSString *)time {
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = [UIColor whiteColor];
        _titleLabel = [YYLabel new];
        ...
        _detailLabel.font = SYSTEM_FONT(11);
        _detailLabel.textColor = UIColorHex(999999);
        _detailLabel.text = [NSString stringWithFormat:@"%@开始",time];
        [self addSubview:_detailLabel];
        CAShapeLayer *layer = [CAShapeLayer layer];
        layer.frame =  CGRectMake(0,kMenuButtonHeight-0.3,self.width,0.3);
        layer.backgroundColor = UIColorHex(0xEFEAE5).CGColor;
