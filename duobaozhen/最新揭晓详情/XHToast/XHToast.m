...
- (id)initWithText:(NSString *)text image:(UIImage *)image{
    if (self = [super init]) {
        ...
        UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(rect.size.height+ 30, 0,rect.size.width + 40, rect.size.height+ 20)];
        ...
        _contentView = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, textLabel.frame.size.width + 10 + imageview.frame.size.width, textLabel.frame.size.height)];
...
+ (void)showCenterWithText:(NSString *)text image:(UIImage *)image duration:(CGFloat)duration{
    XHToast *toast = [[XHToast alloc] initWithText:text image:image];
    [toast setDuration:duration];
    [toast show];
}
#pragma mark-上方显示
+ (void)showTopWithText:(NSString *)text image:(UIImage *)image{
    
    [XHToast showTopWithText:text image:image topOffset:100.0f duration:ToastDispalyDuration];
}
+ (void)showTopWithText:(NSString *)text image:(UIImage *)image duration:(CGFloat)duration
{
     [XHToast showTopWithText:text image:image  topOffset:100.0f duration:duration];
}
+ (void)showTopWithText:(NSString *)text image:(UIImage *)image topOffset:(CGFloat)topOffset{
    [XHToast showTopWithText:text image:image topOffset:topOffset duration:ToastDispalyDuration];
}

+ (void)showTopWithText:(NSString *)text image:image topOffset:(CGFloat)topOffset duration:(CGFloat)duration{
    XHToast *toast = [[XHToast alloc] initWithText:text image:image];
    [toast setDuration:duration];
...
+ (void)showBottomWithText:(NSString *)text image:(UIImage *)image bottomOffset:(CGFloat)bottomOffset duration:(CGFloat)duration{
    XHToast *toast = [[XHToast alloc] initWithText:text image:image];
    [toast setDuration:duration];
