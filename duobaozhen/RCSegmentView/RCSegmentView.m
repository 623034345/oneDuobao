...
- (instancetype)initWithFrame:(CGRect)frame controllers:(NSArray *)controllers titleArray:(NSArray *)titleArray ParentController:(UIViewController *)parentC
{
    if ( self=[super initWithFrame:frame  ])
    {
        ...
        for (int i=0;i<self.controllers.count;i++)
        {
            ...
            contr.view.frame=CGRectMake(i*frame.size.width, 0, frame.size.width,frame.size.height - 40);
...
//            [btn setBackgroundImage:GetColorFromHex(0xffff6c00) forState:UIControlStateSelected];
